...
export function buildCustomerConfirmEmail({ order, baseUrl, delayMinutes = 10 }) {
  ...
  const extraNotesBlock = order.extra_notes
    ? `<tr><td style="padding-top:8px;">
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#1e1d1a;border:1px solid #4a4640;border-radius:14px;">
          <tr><td style="padding:14px 18px;">
            ${sectionLabel(t.email_extra_notes)}
            <p style="margin:0;color:#b8b4ae;font-size:13px;line-height:1.5;">${escapeMultiline(order.extra_notes)}</p>
          </td></tr>
        </table>
      </td></tr>`
    : ''
  ...
  <tr><td align="center" style="padding:44px 16px;">
    <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
      ...
      <tr><td style="padding-bottom:20px;">
        ...
        ${order.extra_notes ? `<p style="margin:12px 0 0;color:#b8b4ae;font-size:13px;"><strong style="color:#f0ede8;">${t.email_bs_extra_notes}</strong> ${escapeMultiline(order.extra_notes)}</p>` : ''}
      </td></tr>
