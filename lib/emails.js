...
export function buildCustomerConfirmEmail({ order, baseUrl, delayMinutes = 10 }) {
  ...
  return {
    subject: `Order confirmation — ${order.butiksnavn}${order.revision > 0 ? ` (revision ${order.revision})` : ''}`,
    html: `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8"/>
      ...
      <tr><td style="padding-bottom:32px;">
        ...
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#1e1d1a;border:1px solid #4a4640;border-radius:16px;">
          ${order.delivery_date ? `<tr><td style="padding:16px 18px 0;">
            <p style="margin:0 0 5px;font-family:${FONT_MONO};font-size:11px;font-weight:500;letter-spacing:0.1em;text-transform:uppercase;color:#7a7672;">Delivery date</p>
            <p style="margin:0;color:#b8b4ae;font-size:13px;">${escapeHtml(new Date(order.delivery_date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }))}</p>
          </td></tr>` : ''}
  ...
  const produkterList = (order.produkter || [])
    .map(p => {
      ...
      return `<li style="margin-bottom:6px;color:#f0ede8;font-size:14px;">${escapeHtml(p.type)}${opts ? ' — ' + opts : ''}: <strong>${qtyText}</strong>` +
        (p.comment ? `<br><span style="color:#b8b4ae;font-size:13px;">Comment: ${escapeHtml(p.comment)}</span>` : '') +
      `</li>`
    })
    .join('')
  ...
  <tr><td align="center" style="padding:44px 16px;">
    <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
      ...
      <tr><td style="padding-bottom:20px;">
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#1e1d1a;border:1px solid #4a4640;border-radius:16px;">
          <tr><td style="padding:18px;">
            ...
            ${order.delivery_date ? `<p style="margin:0 0 8px;color:#b8b4ae;font-size:13px;"><strong style="color:#f0ede8;">Delivery date:</strong> ${escapeHtml(new Date(order.delivery_date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }))}</p>` : ''}
      ...
      ${(order.uploadLinks && order.uploadLinks.length) ? `<tr><td style="padding-bottom:20px;">
        ...
        <p style="margin:6px 0 0;font-size:11px;color:#5a5652;">Download links valid for 7 days.</p>
      </td></tr>` : ''}
