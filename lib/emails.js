  ...
  const produkterRows = (order.produkter || [])
    .map(p => {
      ...
      return `<tr>
        ...
        <td style="padding:10px 14px;border-bottom:1px solid #2e2e2e;color:#b8b4ae;text-align:right;font-size:14px;">${escapeHtml(String(p.antal))} pcs</td>
      </tr>`
    })
    .join('')
...
export function buildBrandsurfaceEmail({ order }) {
  const produkterList = (order.produkter || [])
    .map(p => {
      const format = p.format ? ` — ${escapeHtml(p.format)}` : ''
      return `<li style="margin-bottom:6px;color:#f0ede8;font-size:14px;">${escapeHtml(p.type)}${format}: <strong>${escapeHtml(String(p.antal))} pcs</strong></li>`
    })
    .join('')
